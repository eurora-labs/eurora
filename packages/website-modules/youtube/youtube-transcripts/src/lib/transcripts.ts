 ...
 */
async function raiseHttpErrors(response: Response, videoId: string): Promise<Response> {
	if (!response.ok) {
		const text = await response.text();
		// Throw a more specialized error
		throw new YouTubeRequestFailed(videoId, `Status: ${response.status}, Body: ${text}`);
	}
	return response;
}
...
export class TranscriptListFetcher {
	...
	private async _fetchHtml(videoId: string): Promise<string> {
		const url = WATCH_URL.replace('{video_id}', encodeURIComponent(videoId));
...
export class TranscriptList {
	...
	public static build(
		...
		for (const caption of captionsJson.captionTracks) {
			...
			const transcriptMap = isAsr ? generatedTranscripts : manuallyCreatedTranscripts;
...
export class Transcript {
	...
	public async fetch(preserveFormatting = false): Promise<Array<TranscriptLine>> {
	...
	public parse(plainData: string): Array<TranscriptLine> {
		...
		return texts
			...
			.map((elem) => {
				...
				const cleanedText = rawText ? rawText.replace(this._htmlRegex, '') : '';
