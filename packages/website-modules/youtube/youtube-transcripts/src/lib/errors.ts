...
`;
	...
	constructor(videoId: string, causeMsg?: string) {
		const formatted = CouldNotRetrieveTranscript._buildErrorMessage(videoId, causeMsg);
		super(formatted);
	...
	private static _buildErrorMessage(videoId: string, causeMsg?: string): string {
		...
		if (causeMsg) {
			message +=
				CouldNotRetrieveTranscript.CAUSE_MESSAGE_INTRO.replace('{cause}', causeMsg) +
...
export class FailedToCreateConsentCookie extends CouldNotRetrieveTranscript {
	constructor(videoId: string) {
		super(videoId, 'Failed to automatically give consent to saving cookies');
	}
}
