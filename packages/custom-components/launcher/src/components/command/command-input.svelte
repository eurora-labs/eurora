...
<div class=" flex items-center border-none px-3" data-command-input-wrapper="">
	...
	<CommandPrimitive.Input
		class={cn(
			' custom-input flex w-full rounded-md border-none bg-transparent py-3 text-base shadow-none outline-none focus:border-transparent focus:ring-0 disabled:cursor-not-allowed disabled:opacity-50 md:text-sm',
			className
		)}
	...
	>
		{#snippet child({ props })}
			<!-- <input class="text-black" bind:value {...props} /> -->
