...
├── shared/        # bindings.ts (generated by `pnpm specta`)
├── util/          # log, session-id
└── word/          # Office.js wrappers (Word.run, body.text, etc.)
```

The wire types in `src/shared/bindings.ts` are generated from the Rust crate
`crates/app/euro-office` via the workspace-level `euro-codegen` orchestrator.
Regenerate them whenever bridge or office types change:
