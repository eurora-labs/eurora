...
(function () {
    ...
    if (chrome.runtime.id !== 'oemmndcbldboiebfnladdacbdfmadadm') {
        ...
        console.warn(
            'Disabled telemetry because this is not an official build.',
        );
        return;
    }
    ...
    chrome.storage.session.get({ didPingCheck: false }, async (items: any) => {
    ...
    function doSendPing() {
        chrome.storage.local.get(localStorage, (items) => {
            ...
            var lastTime = parseInt(localStorage.telemetryLastTime as any) || 0;
            var wasUpdated = didUpdateSinceLastCheck();
            if (
                !wasUpdated &&
                Date.now() - lastTime < MINIMUM_TIME_BETWEEN_PING
            ) {
                return;
            }
    ...
    function didUpdateSinceLastCheck() {
        var chromeVersion = /Chrome\/(\d+)\./.exec(navigator.userAgent) as any;
    ...
    function getLoggingPref(callback) {
        ...
        function next(result) {
            ...
            chrome.storage[storageAreaName].get(
                'disableTelemetry',
                function (items) {
