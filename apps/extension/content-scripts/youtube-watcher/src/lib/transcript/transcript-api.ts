...
export class Transcript {
  ...
  async fetch(preserveFormatting: boolean = false): Promise<FetchedTranscript> {
    try {
      ...
      if (!response.ok) {
        throw new YouTubeTranscriptError(`HTTP ${response.status}: ${response.statusText}`, this.videoId);
      }
  ...
  translate(languageCode: string): Transcript {
    ...
    const targetLanguage = this.translationLanguages.find(lang => lang.languageCode === languageCode);
    if (!targetLanguage) {
      throw new YouTubeTranscriptError(`Translation language ${languageCode} not available`, this.videoId);
    }
...
export class TranscriptList {
  ...
  findTranscript(languageCodes: string[]): Transcript {
    return this.findTranscriptInternal(languageCodes, [this.manuallyCreatedTranscripts, this.generatedTranscripts]);
  }
  ...
  private findTranscriptInternal(languageCodes: string[], transcriptMaps: Map<string, Transcript>[]): Transcript {
...
export class YouTubeTranscriptApi {
  async fetch(videoId: string, languages: string[] = ['en'], preserveFormatting: boolean = false): Promise<FetchedTranscript> {
  ...
  private async fetchInnertubeData(videoId: string, apiKey: string): Promise<InnertubeData> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new YouTubeTranscriptError(`HTTP ${response.status}: ${response.statusText}`, videoId);
      }
    ...
    if (playabilityStatus.status !== 'OK') {
      ...
      if (playabilityStatus.status === 'ERROR' && reason === 'This video is unavailable') {
        if (videoId.startsWith('http://') || videoId.startsWith('https://')) {
          throw new YouTubeTranscriptError('Invalid video ID - use video ID, not URL', videoId);
        }
        throw new VideoUnavailableError(videoId);
      }
      
      throw new YouTubeTranscriptError(`Video unplayable: ${reason || 'Unknown reason'}`, videoId);
    }
  }
}