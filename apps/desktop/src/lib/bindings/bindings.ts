...
export type Claims = { sub: string; username: string; email: string; exp: bigint; iat: bigint; token_type: string; role: Role; aud?: string }
...
export type MessageTreeNodeView = { id: string; parent_message_id: string | null; message_type: string; content: string; level: number; sibling_count: number; sibling_index: number; assets: MessageAssetChip[] | null; reasoning_blocks: ReasoningBlock[] | null }
...
export type Query = { text: string; assets: string[]; parent_message_id: string | null }
...
get_message_tree: (threadId: string, startLevel: number, endLevel: number) => Promise<MessageTreeNodeView[]>, 
