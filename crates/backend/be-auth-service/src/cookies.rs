...
//! `eu_access` (HttpOnly), the refresh JWT in `eu_refresh` (HttpOnly,
//! path-scoped to `/auth`), and a separate JS-readable random
//! `eu_csrf` cookie underwrites the double-submit CSRF defense. This
//! module owns the names, attributes, and `Set-Cookie` builders so
//! every handler emits a consistent shape.
//!
...
pub const CSRF_COOKIE: &str = "eu_csrf";
...
/// HTTP header the SPA echoes the CSRF cookie back into for the
/// double-submit check.
...
pub struct CookieConfig {
    ...
    /// bearer-mode. The set is also consulted by the CSRF guard.
...
impl CookieConfig {
    pub fn from_env() -> Self {
        ...
        let web_origins: HashSet<String> = std::env::var("WEB_ALLOWED_ORIGINS")
...
/// Generate a fresh CSRF token: 256 bits of OS randomness, base64-url
/// encoded so it can travel in headers without escaping.
...
/// Build the CSRF cookie. Crucially **not** HttpOnly — the SPA reads
/// it and echoes the value back as `X-CSRF-Token` for double-submit.
pub fn csrf_cookie(cfg: &CookieConfig, value: String, max_age_secs: i64) -> Cookie<'static> {
    base_cookie(
        cfg,
        CSRF_COOKIE,
        value,
...
pub fn clear_all(cfg: &CookieConfig, jar: CookieJar) -> CookieJar {
    jar.add(removal_cookie(cfg, ACCESS_COOKIE, ACCESS_COOKIE_PATH))
        ...
        .add(removal_cookie(cfg, CSRF_COOKIE, ACCESS_COOKIE_PATH))
}
...
mod tests {
    ...
    fn csrf_token_is_url_safe() {
        ...
        for byte in token.bytes() {
            assert!(
                ...
                "non-url-safe byte {byte:?} in {token}"
            );
        }
    }
    ...
    fn csrf_cookie_is_not_http_only() {
        let cfg = config_with_origin("https://www.eurora-labs.com");
        let cookie = csrf_cookie(&cfg, "tok".into(), 3600);
