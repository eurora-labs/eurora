...
impl From<&euro_llm_core::Message> for OpenAIMessage {
    fn from(message: &euro_llm_core::Message) -> Self {
        ...
        let content = match &message.content {
            euro_llm_core::MessageContent::Text(text) => {
            ...
            euro_llm_core::MessageContent::Multimodal(parts) => {
                let content_array: Vec<serde_json::Value> = parts
                    .iter()
                    .map(|part| match part {
                        euro_llm_core::ContentPart::Text { text } => serde_json::json!({
                            "type": "text",
                            "text": text
                        }),
                        euro_llm_core::ContentPart::Image {
                            image_source,
                            detail,
                        } => {
                        ...
                        euro_llm_core::ContentPart::Audio { audio_url, format } => {
                            serde_json::json!({
                                "type": "audio",
                                "audio": {
                                    "mime_type": format
...
impl From<&euro_llm_core::Tool> for OpenAITool {
