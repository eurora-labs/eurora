...
impl From<ContentPart> for ProtoContentPart {
    fn from(part: ContentPart) -> Self {
        let part_type = match part {
            euro_llm::ContentPart::Text { text } => ProtoPartType::Text(ProtoTextPart { text }),
            ...
            euro_llm::ContentPart::Audio { audio_url, format } => {
...
impl From<MessageContent> for ProtoMessageContent {
    fn from(content: MessageContent) -> Self {
        let content_type = match content {
            euro_llm::MessageContent::Text(text) => ProtoContentType::Text(text),
