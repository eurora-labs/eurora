...
impl From<ContentPart> for ProtoContentPart {
    fn from(part: ContentPart) -> Self {
        let part_type = match part {
            euro_llm::core::types::ContentPart::Text { text } => {
                ProtoPartType::Text(ProtoTextPart { text })
            }
            euro_llm::core::types::ContentPart::Image {
                image_source,
                detail,
            } => {
            ...
            euro_llm::core::types::ContentPart::Audio { audio_url, format } => {
...
impl From<MessageContent> for ProtoMessageContent {
    fn from(content: MessageContent) -> Self {
        let content_type = match content {
            euro_llm::core::types::MessageContent::Text(text) => ProtoContentType::Text(text),
            euro_llm::core::types::MessageContent::Multimodal(parts) => {
            ...
            euro_llm::core::types::MessageContent::Tool(tool_content) => {
                let tool_calls = tool_content
                    .tool_calls
                    .unwrap_or_default()
                    .into_iter()
