...
use euro_llm_core::{ChatResponse, CompletionResponse, FinishReason, Metadata, Usage};
...
impl ChatResponse for OllamaChatResponseWrapper {
    ...
    fn tool_calls(&self) -> Option<Vec<euro_llm_core::ToolCall>> {
...
impl From<&euro_llm_core::Message> for OllamaMessage {
    fn from(message: &euro_llm_core::Message) -> Self {
        let role = match message.role {
            ...
            euro_llm_core::Role::Tool => "tool".to_string(),
        };

        let content = match &message.content {
            euro_llm_core::MessageContent::Text(text) => text.clone(),
            euro_llm_core::MessageContent::Multimodal(parts) => {
                // Extract text parts and collect images
                let text_parts: Vec<String> = parts
                    .iter()
                    .filter_map(|part| match part {
                        euro_llm_core::ContentPart::Text { text } => Some(text.clone()),
                        _ => None,
                    })
                    .collect();
                text_parts.join("\n")
...
mod tests {
    ...
    fn test_message_conversion() {
        let core_message = euro_llm_core::Message::user("Hello, world!");
