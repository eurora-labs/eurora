...
fn test_dict_int_op_add() {
    ...
    let result = dict_int_op_json(&left, &right, |x, y| x + y, 0, 100).unwrap();
...
fn test_dict_int_op_subtract() {
    ...
    let result = dict_int_op_json(&left, &right, |x, y| (x - y).max(0), 0, 100).unwrap();
...
fn test_dict_int_op_max_depth_exceeded() {
    ...
    let result = dict_int_op_json(&left, &right, |x, y| x + y, 0, 2);
...
fn test_dict_int_op_invalid_types() {
    ...
    let result = dict_int_op_json(&left, &right, |x, y| x + y, 0, 100);
