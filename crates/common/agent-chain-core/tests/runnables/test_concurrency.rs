...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn test_batch_as_completed_concurrency() {
    let running_tasks = Arc::new(AtomicUsize::new(0));
    let max_running_tasks = Arc::new(AtomicUsize::new(0));
    ...
    let config = RunnableConfig::builder()
        .max_concurrency(max_concurrency)
        .build();
    let mut stream = runnable.batch_as_completed(
        (0..num_tasks as i32).collect(),
        Some(ConfigOrList::from(config)),
        false,
    );
...
async fn test_batch_empty_input() {
    ...
    let results = runnable.batch(vec![], None, false).await;
    assert!(results.is_empty());
}

#[tokio::test]
async fn test_batch_single_item_no_threading() {
    let runnable = RunnableLambda::builder().func(|x: i32| Ok(x * 2)).build();
    let results = runnable.batch(vec![5], None, false).await;
...
async fn test_batch_preserves_order() {
    ...
    let results = runnable.batch(inputs.clone(), None, false).await;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn test_batch_with_return_exceptions() {
    let runnable = RunnableLambda::builder()
        .func(|x: i32| {
            if x % 2 == 0 {
    ...
    let results = runnable.batch(vec![1, 2, 3, 4], None, true).await;
...
#[tokio::test]
async fn test_batch_no_concurrency_limit() {
    let runnable = RunnableLambda::builder().func(|x: i32| Ok(x + 1)).build();
    let results = runnable.batch((0..20).collect(), None, false).await;
...
#[tokio::test]
async fn test_batch_concurrency_of_one() {
    ...
    let config = RunnableConfig::builder().max_concurrency(1).build();
    let results = runnable
        .batch((0..5).collect(), Some(ConfigOrList::from(config)), false)
        .await;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn test_batch_as_completed_returns_all() {
    let runnable = RunnableLambda::builder()
        .func(|x: i32| {
    ...
    let mut stream = runnable.batch_as_completed((0..5).collect(), None, false);
