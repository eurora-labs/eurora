...
// ---------------------------------------------------------------------------
// Standalone uninstall for the `office-addin-clean` CLI tool.
//
// The CLI cannot use `uninstall_for_app` because that requires a running
// Tauri runtime. We resolve the same per-OS paths via `dirs` so the cleanup
// converges on the exact same state — file path and registry subkey — that
// `install_for_app` writes to, by construction.
// ---------------------------------------------------------------------------
