//! Bundle layout, persisted add-in GUID, and manifest template rendering.
...
pub(super) const RESOURCE_SUBDIR: &str = "office-addin";
...
pub(super) fn resolve_paths<R: Runtime>(app: &AppHandle<R>) -> Result<AddinPaths> {
...
fn load_or_create_addin_id<R: Runtime>(app: &AppHandle<R>) -> Result<String> {
...
fn manifest_params<R: Runtime>(app: &AppHandle<R>, paths: &AddinPaths) -> Result<ManifestParams> {
...
fn render_manifest(template: &str, params: &ManifestParams) -> Result<String> {
...
/// Render the bundled template using the current app's params.
