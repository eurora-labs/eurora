...
impl AuthApi for AuthApiImpl {
    async fn get_login_token<R: Runtime>(
        ...
        // `WEB_URL` is baked into the binary by `build.rs` and
        // injected into the process env by `euro_tauri::load_env`,
        // so a runtime miss here means someone called this entry
        // point before `main` ran.
