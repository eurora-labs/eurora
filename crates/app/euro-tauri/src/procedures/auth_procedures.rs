...
#[tauri::command]
#[specta::specta]
pub async fn auth_get_login_token(app_handle: AppHandle) -> Result<LoginToken, String> {
    ...
    let base_url = std::env::var("WEB_URL").ctx("WEB_URL not in process env")?;
    let mut url = Url::parse(&format!("{base_url}/login")).ctx("Invalid WEB_URL")?;
    url.query_pairs_mut()
...
#[tauri::command]
#[specta::specta]
pub async fn auth_poll_for_login(app_handle: AppHandle) -> Result<bool, String> {
...
#[tauri::command]
#[specta::specta]
pub async fn auth_resend_verification_email(app_handle: AppHandle) -> Result<(), String> {
    let auth_manager = auth_manager(&app_handle).await?;
    auth_manager
