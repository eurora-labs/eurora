...
pub(crate) struct SettingsEnforceSaveToDisk<'a> {
    config_path: &'a Path,
    snapshot: RwLockWriteGuard<'a, Settings>,
    saved: bool,
}
...
impl SettingsWithDiskSync {
    ...
    pub fn new(config_dir: impl AsRef<Path>) -> Result<Self> {
        ...
        let app_settings = Settings::load(&config_path)?;
    ...
    pub fn watch_in_background(
        &mut self,
        send_event: impl Fn(Settings) -> Result<()> + Send + Sync + 'static,
    ) -> Result<()> {
            ...
            loop {
                match rx.recv() {
                    Ok(Ok(Event {
                        ...
                        if let Ok(update) = Settings::load(&config_path) {
