#!/usr/bin/env bash

set -euo pipefail

if [[ ! " $* " == *" -p "* && ! " $* " == *" --package "* ]]; then
    set -- "$@" --workspace
fi

set -x
# Default to `just cargo` so build scripts that read process env (e.g.
# euro-{mobile,tauri,endpoint,settings}/build.rs requiring WEB_URL /
# BACKEND_URL) get the localhost defaults exported at the top of the
# justfile. CI sets `CARGO=cargo` explicitly because the workflow
# already exports the vars via `env:`/`secrets.*`.
${CARGO:-just cargo} clippy "$@" --release --all-targets --all-features -- --deny warnings

# If local, run other checks if we have the tools installed.
if [[ -z "${GITHUB_ACTIONS+x}" ]]; then
    which cargo-machete >/dev/null 2>&1 || exit 0
    cargo machete

    which typos >/dev/null 2>&1 || exit 0
    typos --config typos.toml
fi
